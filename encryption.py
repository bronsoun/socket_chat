...
class DiffieHellman:
    def __init__(self, group=14):
        if group in primes:
            ...
            raise Exception("Некорректный выбор ключей")
        # генерация рандомного секрtnного ключа (степени)
        self.__a = int(binascii.hexlify(os.urandom(32)), base=16)

    def gen_public_key(self):
        # высчитываем G^a mod p
        return pow(self.g, self.__a, self.p)

    def check_other_public_key(self, pub_key):
        # проверка правильности ключа (больше двух и меньше модуля на 2)
    ...
    def gen_shared_key(self, pub_key):
        ...
        if self.check_other_public_key(pub_key):
            ...
            raise Exception("Некорректный публичный ключ")


class AESCipher(object):
    def __init__(self, key):
        self.bs = AES.block_size # устанавливает размер блока - 128 бит
        self.key = hashlib.sha256(key.encode()).digest()

    def encrypt(self, raw):
        raw = self._pad(raw)
        iv = Random.new().read(AES.block_size)
        # MODE_CBC - cipher_mode_chaining - каждый блок текста XORится
        # с предыдущим блоком зашифрованного текста перед шифрованием
        # A block cipher uses a symmetric key to encrypt data of fixed and very short length
        # (the block size), such as 16 bytes for AES. In order to cope with data of arbitrary
        # length, the cipher must be combined with a mode of operation.
        cipher = AES.new(self.key, AES.MODE_CBC, iv)
        # the Initialization Vector. A piece of data unpredictable to adversaries.
    ...
    def _pad(self, s):
        # для заполнения сообщения до размера блока
